#!/bin/sh
source /mnt/SDCARD/System/usr/miyoo/scripts/common_launcher.sh
cpufreq.sh ondemand 5 7

Rom="$@"
RomPath=$(dirname "$1")
romName=$(basename "$@")
romNameNoExtension=${romName%.*}

if [ "$romName" = "° Run ScummVM.launch" ]; then
	mv "$Rom" "$RomPath/$romNameNoExtension.squashfs"
	rm "/mnt/SDCARD/Roms/SCUMMVM/SCUMMVM_cache7.db"
fi

if [ "$romName" = "° Import ScummVM Games.launch" ]; then
	Current_Theme=$(/usr/miyoo/bin/jsonval theme)
	Current_bg="$Current_Theme/skin/background.png"
	if [ ! -f "$Current_bg" ]; then
		Current_bg="/mnt/SDCARD/System/res/skin/transparent.png"
	fi
	/mnt/SDCARD/System/usr/miyoo/scripts/infoscreen.sh -i "$Current_bg" -m "Importing ScummVM games." -t 2
	sh "$Rom"
	sleep 0.3
	exit
fi


cd $RA_DIR/

HOME=$RA_DIR/ $RA_DIR/ra64.miyoo -v -L $RA_DIR/.retroarch/cores/scummvm_libretro.so "$@"
